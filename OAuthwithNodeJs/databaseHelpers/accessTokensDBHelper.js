...
function saveAccessToken(accessToken, userID, callback) {
  ...
  const getUserQuery =  `INSERT INTO oauth_access_tokens (access_token, user_id) VALUES ("${accessToken}", ${userID}) ON DUPLICATE KEY UPDATE access_token = "${accessToken}";`
...
function getUserIDFromBearerToken(bearerToken, callback){
  ...
  const getUserIDQuery = `SELECT * FROM oauth_access_tokens WHERE access_token = '${bearerToken}';`
